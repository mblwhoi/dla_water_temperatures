<?php # Show_yearstat.php
$update_date = "April 21, 2010";
$yr_wanted = $_POST['year'];
$page_title = "Historic WHOI Pier Water Temperature Statistics for $yr_wanted";
...
$query1 = "SELECT min(temperature) AS minimum, max(temperature) AS maximum, avg(temperature) AS average FROM h2o_temps WHERE year=$yr_wanted AND temperature != 'NULL'";

//Run the query
$result1 = $rs1 = $db->executeQuery($query1);


if ($rs1->next()) { // If it ran OK, get the dates for min and display
    $result1 = $rs1->getCurrentValuesAsHash();
    echo '<center><h2>Water Temperature Statistics for ' . $yr_wanted . '</h2></center>';
    $minimum = $result1['minimum'];
    
    $query2 = "SELECT month,date FROM h2o_temps WHERE year = $yr_wanted AND temperature = $minimum ORDER BY month,date";
    $result2 = $db->executeQuery($query2);
    
    echo '<hr> <b>The lowest temperature in ' . $yr_wanted . ' was ' . $result1['minimum']
         . ' degrees. It occured on the following date(s): </b><ul>';
    while ($result2->next()) {
        $row = $result2->getCurrentValuesAsHash();
        echo '<li>' . $months[$row['month']] . ' ' . $row['date'] . '</li>';
    }
    echo '</ul>';
    $maximum = $result1['maximum'];
    
    $query3 = "SELECT month,date FROM h2o_temps WHERE year = $yr_wanted AND temperature = $maximum ORDER BY month,date";
    $result3 = $db->executeQuery($query3);
    
    echo '<hr> <b>The highest temperature in ' . $yr_wanted . ' was ' . $result1['maximum']
          . ' degrees. It occured on the following date(s): </b><ul>';
    while ($result3->next()) {
    ...
    echo '</ul>';
    echo '<hr> <b>The average temperature in ' . $yr_wanted . ' was ' . sprintf("%.1f", $result1['average']) . ' degrees.</b>';
