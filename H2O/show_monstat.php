...
$page_title = "Historic WHOI Pier Water Temperature Statistics for $months[$mon_wanted]";
...
$query1 = "SELECT min(temperature) AS minimum, max(temperature) AS maximum, avg(temperature) AS average FROM h2o_temps WHERE month=$mon_wanted AND temperature != 'NULL'";
//Run the query
...
if ($rs1->next()) { // If it ran OK, get the dates for min and display
    ...
    echo '<center><h2>Water Temperature Statistics for ' . $months[$mon_wanted] . '</h2></center>';
    $minimum = $result1['minimum'];
    
    $query2 = "SELECT year,month,date FROM h2o_temps WHERE month = $mon_wanted AND temperature = $minimum ORDER BY year,month,date";
    $result2 = $db->executeQuery($query2);
    
    
    echo '<hr> <b>The lowest temperature in ' . $months[$mon_wanted] . ' was ' . $result1['minimum']
         . ' degrees. It occured on the following date(s): </b><ul>';
    while ($result2->next()) {
        $row = $result2->getCurrentValuesAsHash();
        echo '<li>' . $row['year'] . ': ' . $months[$row['month']] . ' ' . $row['date'] . '</li>';
    }
    
    echo '</ul>';
    $maximum = $result1['maximum'];
    
    $query3 = "SELECT year,month,date FROM h2o_temps WHERE month = $mon_wanted AND temperature = $maximum ORDER BY year,month,date";
    $result3 = $db->executeQuery($query3);
    
    echo '<hr> <b>The highest temperature in ' . $months[$mon_wanted] . ' was ' . $result1['maximum']
          . ' degrees. It occured on the following date(s): </b><ul>';
    while ($result3->next()) {
    ...
    echo '</ul>';
    echo '<hr> <b>The average temperature in ' . $months[$mon_wanted] . ' was ' . sprintf("%.1f", $result1['average']) . ' degrees.</b>';
